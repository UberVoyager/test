...
if(getCookie("policyLinearDisplay")==""){
	setCookie("policyLinearDisplay",false,1000);
}
if(getCookie("unitLinearDisplay")==""){
	setCookie("unitLinearDisplay",false,1000);
}
if(getCookie("traitLinearDisplay")==""){
	setCookie("traitLinearDisplay",false,1000);
}
G.PARTY=0;
...
G.ocean;
G.tabs=
	[ //div : which div to empty+hide or display when tab is toggled //update : which system's update to call when toggling on
...
func:function(){
	...
	UPDATELOG=[
	{date:'2013-2014',title:'<font color="pink">Prototype</font>',text:[
	'started working on a prototype for a cross between Civilization and an idle game after the sudden success of my game, <a href="//orteil.dashnet.org/cookieclicker/" target="_blank">Cookie Clicker</a>.'
	,'encountered some issues with gameplay design and ended up putting the project on hold']},
	{date:'2015',title:'<font color="pink">Prototype, part II</font>',text:['the broken, buggy prototype has been <a href="//orteil.dashnet.org/experiments/legacy/" target="_blank">put online for all to see</a>']},
	{date:'2016',title:'<font color="pink">Return of the Prototype</font>',text:['motivated by the positive response to the previous prototype, a new version was started from scratch with fresh new ideas']},
	{date:'3/23/2017',title:'<font color="#b3ffbf">Alpha launch</font>',text:['a playable alpha is launched publicly','at this point, the game features 54 technologies, 42 units, and 91 resources']},
	{date:'3/24/2017',title:'<font color="#b3ffbf">Alpha patch</font>',text:['added an outline to your explored territory on the map','hunters dying, quarries/mines collapsing and scouts getting lost should no longer disproportionately harm your population; those effects have also been made less frequent','people get sick less often','added a new policy to control birth rate','mines can now mine for salt','resources now display how much you\'re gaining and losing every tick']},
	{date:'3/25/2017',title:'<font color="#b3ffbf">Alpha patch 2</font>',text:['units are now queued for automatic purchase rather than being purchased directly; this allows them to be automatically replaced should they be harmed','fixed bug with units getting lost or wounded way too much (maybe for good this time?)','buildings no longer require you to have available tools and workers to build them, as this was confusing and not very fun','graves now decay over time to make room for more; architects now have an "undertaker" mode that automatically creates graves if there are unburied corpses','material and food decay were slowed and storage units have bigger capacity']},
	{date:'3/25/2017',title:'<font color="#b3ffbf">Alpha patch 3</font>',text:['units can now be active or inactive; a building that lacks workers, or a crafter that lacks its tools, will simply go inactive instead of disappearing, and will be made active again when the requirements are met; units are inactive when they\'re first created and when they\'ve just been set to a new mode','the mausoleum can be completed again','graves should behave better']},
	{date:'3/26/2017',title:'<font color="#b3ffbf">Alpha patch 4</font>',text:['fixed many miscellaneous bugs, hopefully','scouting and exploring speed now properly depends on how many wanderers or scouts you have','gathering is now soft-capped by natural resources; this means having many gathering units but few tiles won\'t have optimal results','removing units now removes the idle ones first']},
	{date:'3/26/2017',title:'<font color="#b3ffbf">Alpha patch 5</font>',text:['your people will no longer be completely apathetic and neutrally healthy from some bug with consuming food','fire pits warm more people','clothiers no longer need to know leatherworking to sew grass clothing','happiness and health sources are detailed more explicitly','many messages now have icons']},
	{date:'3/28/2017',title:'<font color="#b3ffbf">Alpha patch 6</font>',text:['unit modes now have icons','added custom bulk-buying in units','gathering was reworked, expect different rates for resource production','workers dying while working probably won\'t result in ghost workers anymore','units have innate priorities in the context of being created and acting, with food-producing units going first','happier people now produce more babies, while unhappy people just aren\'t feeling it as much','corpses decay slowly']},
	{date:'7/16/2019',title:'An overhaul called "Magix"~pelletsstarPL',text:['total game overhaul since 2019. For more detailed Magix update log check <u>Magix</u> tab.','<font color="red">This log has been added by modder not by an original dev</font>']},
];
if(Date.getMonth==3 && Date.getDate==1)UPDATELOG+={date:'4/1/1990',title:'NES Prototype',text:['Old , super buggy damned prototype for NES. I\'d rather keep it for myself than sharing it.','randomly crashes','sometimes lags']}
	...
	'<div class="par">The idea is to keep releasing incremental updates that add new technological eras or expand on existing features. This may take a while. <small>I heard this tab contains small clickable. Scroll down</small><br>I also added some colours over here ~ pelletsstarPL</div>';
...
G.legacyBonuses.push(
	...
	G.update['trait']=function()
	{
		...
		str+='<div style="position:absolute;z-index:0;top:0px;left:0px;right:0px;text-align:right;"><div class="flourishL"></div>'+
			G.button({id:'displayTrait',
					text:'<span style="position:relative;width:20px;margin-left:-4px;margin-right:0px;z-index:10;font-weight:bold;">Linear display:'+(getCookie("traitLinearDisplay")=="true" ? '<font color="#bbffbb" id="toggledp">ON</font>' : '<font color="#ffbbbb" id="toggledp">OFF</font>')+'</span>',
					tooltip:'Toggle between displaying trait categories in bulk or one category per line.',
					onclick:function(){
						if(getCookie("traitLinearDisplay")=='true')setCookie("traitLinearDisplay",'false',1000);
						else if(getCookie("traitLinearDisplay")=='false') setCookie("traitLinearDisplay",'true',1000);
						G.updateDisplayToggle();
						G.update['trait']();
					},
				});
				str+='</div>';
		l('traitDiv').innerHTML=
			...
			G.button({tooltip:'Lets you change the names of various things,<br>such as your civilization, your people, and yourself.',text:'Rename civilization',onclick:function(e){G.dialogue.popup(function(div){
				...
				if(G.modsByName["Default dataset"]){
					...
					str+='<div class="fancyText par">Your wizards also opened the portal to the island that is called:'+G.field({text:getCookie("island"),tooltip:"This is your name for Plain Island",oninput:function(val){setCookie("island",val,1000)}})+".<br><br><br>";
				}
		...
		{	
			...
			'<div class="fullCenteredOuter"><div class="fullCenteredInner"><div id="extraCultureStuff" style="text-align:center;margin-bottom:8px;"></div><div id="traitBox" class="thingBox"></div></div></div></br></br></br>'+
			'<Div class="framed bgMid" style="width:240px"><div class="category" style="display:inline-block;width:240px">'+strByCat[G.knowCategories[iC].id]+'</div></div></center><br />';continue};
			if(getCookie("traitLinearDisplay")=='true'){
	...
	new G.Policy({
		name:'death messages',
		desc:'Disable/Enable messages informing that some [population,'+(G.modsByName['Default dataset'] ? 'people' : 'elves')+'] have died. <>Note: It will disable ANY death messages regardless of reason of their death.',
		icon:[13,32,'magixmod'],
		cost:{},
		startMode:'on',
		req:{'message filtering':true},
	...
	new G.Policy({
		...
		desc:'Disable/Enable messages that inform you about trait your '+(G.modsByName['Default dataset'] ? 'people' : 'elves')+' have just adopted.',
		icon:[16,32,'magixmod'],
		cost:{},
		startMode:'on',
		req:{'message filtering':true},
	...
	new G.Policy({
		...
		desc:'Disable/Enable messages that inform you how many [population,'+(G.modsByName['Default dataset'] ? 'people' : 'elves')+'] have become [sick]. This also disables infos about recovered [sick] '+(G.modsByName['Elves'] ? 'people' : 'elves')+'.',
		icon:[18,32,'magixmod'],
		cost:{},
		startMode:'on',
		req:{'message filtering':true},
	...
	new G.Policy({
		...
		desc:'Disable/Enable messages that inform you how many [population,'+(G.modsByName['Default dataset'] ? 'people' : 'elves')+'] have become [wounded]. This also disables infos about recovered [wounded] citizens.',
		icon:[21,32,'magixmod'],
		cost:{},
		startMode:'on',
		req:{'message filtering':true},
	...
	new G.Policy({
		...
		desc:'Disable/Enable messages about [housing,homelessness]. <small>Those will always display... ehh. You want turn\'em off I guess.</small> '+(G.modsByName['Elves'] ? 'For this race it also disables messages about reaching cap of [pressure resistance]' : '')+'.',
		icon:[22,32,'magixmod'],
		cost:{},
		startMode:'on',
		req:{'message filtering':true},
...
//skull achiev
	...
	new G.Achiev({
		...
		desc:'Obtain [magic adept] trait. Manage to get 2.1M [magic essences]. This trait only occurs with <b>Evolution of the minds obtained</b>//Obtaining it may unlock a new wonder. ',
		effects:[
	...
	new G.Achiev({
		...
		name:'talented?',
		icon:[32,25,'magixmod'],
		desc:'To get this achievement you need to complete rest achievements in this tier. @<b>Achievement bonus:All crafting units that use land of primary world will use 0.15 less land per 1 piece so if unit uses 3 land it will use 2.55 upon obtain. In addition this bonus applies to [well]s, <b>wheat farms</b>,<b>water filters</b> (0.1 less for Caretaking filter and 0.2 less for Moderation one) and <b>crematoriums</b>.<>Note: Bonus does not apply to paper crafting shacks</b> @In addition completing full row will now make you be able to pick <b>1 of 5</b> techs in research box instead of <b>1 of 4</b>. And... it unlocks new theme!',
		effects:[
	...
	G.tabPopup['legacy']=function()
	{
		...
		if(G.achievByName['i do not want to take things easily'].won>=1){G.achievByName['i do not want to take things easily'].visible=true};
		var str='';var a=0;
		...
		str+='<div class="par">Current amount of adopted traits: <b>'+G.selfUpdatingText(function(){return (G.modsByName['Elves'] ? G.traitN2 : G.traitN);})+'</b></div>';
		str+='<div class="par">Current amount of obtained researches:<b>'+G.selfUpdatingText(function(){return G.techN-G.miscTechN;})+'</b></div>';
	...
	G.updateDisplayToggle=function()
	{
		if (G.tab.id=="policy")
		{
			if(getCookie("policyLinearDisplay")=='true')document.getElementById("toggledp").innerHTML='<font color="#bbffbb" id="toggled">ON</font>';
			else document.getElementById("toggledp").innerHTML='<font color="#FFBBbb" id="toggled">OFF</font>';
		}else if(G.tab.id=="unit"){
			if(getCookie("unitLinearDisplay")=='true')document.getElementById("toggledu").innerHTML='<font color="#bbffbb" id="toggled">ON</font>';
			else document.getElementById("toggledu").innerHTML='<font color="#FFBBbb" id="toggled">OFF</font>';
		}
	};
	...
	if(G.has('policies')){
		...
		str+=
			...
			str+='<div style="position:absolute;z-index:0;top:0px;left:0px;right:0px;text-align:right;"><div class="flourishL"></div>'+
			G.button({id:'displayPolicy',
					text:'<span style="position:relative;width:20px;margin-left:-4px;margin-right:0px;z-index:10;font-weight:bold;">Linear display:'+(getCookie('linearDP')=='true' ? '<font color="#bbffbb" id="toggledp">ON</font>' : '<font color="#ffbbbb" id="toggledp">OFF</font>')+'</span>',
					tooltip:'Toggle between displaying policy categories in bulk or one category per line.',
					onclick:function(){
						if(getCookie('linearDP')=='true')setCookie('linearDP','false',1000);
						else setCookie('linearDP','true',1000);
						G.updateDisplayToggle();
						G.update['policy']();
					},
				});
	...
	G.createDebugMenu=function()
	{
		...
		G.button({text:'<font color="fuschia">Reveal map</font>',tooltip:'Explore the whole map instantly.',onclick:function(){G.revealMap(G.currentMap);}})+
		    '<br /><font color="lime">Debug mode. Magix: Release 54</font>'+
	...
	G.Message=function(obj)
	{
		...
		else if(G.has('primary time measure') && G.has('time measuring 2/2')){
        ...
        if (G.checkPolicy("toggle SFX") == "on") {
          ...
          audio.play();
			...
			'<div style="position:absolute;z-index:0;top:0px;left:0px;right:0px;text-align:right;"><div class="flourishL"></div>'+
				G.button({id:'displayUnit',
					text:'<span style="position:relative;width:20px;margin-left:-4px;margin-right:0px;z-index:10;font-weight:bold;">Linear display:'+(getCookie("unitLinearDisplay")=="true" ? '<font color="#bbffbb" id="toggledu">ON</font>' : '<font color="#ffbbbb" id="toggledu">OFF</font>')+'</span>',
					tooltip:'Toggle between displaying unit categories in bulk or one category per line.',
					onclick:function(){
						if(getCookie("unitLinearDisplay")=='true')setCookie("unitLinearDisplay",'false',1000);
						else if(getCookie("unitLinearDisplay")=='false') setCookie("unitLinearDisplay",'true',1000);
						G.updateDisplayToggle();
						G.update['unit']();
					},
				});
		...
		*/
		...
		for (var iC=0;iC<len;iC++)
		{
			if (strByCat[G.unitCategories[iC].id]!='')
			{
				...
				if(getCookie("unitLinearDisplay")=='true')
					...
					if (type==0)//effects that happen every tick
					{
						if (!effect.every || G.tick%effect.every==0)//.every : effect only triggers every X days
						{
							...
							for (var repI=0;repI<repeat;repI++)
							{
								...
								else if (effect.type=='exploreGatherer')//far foraging policy
								{
										if(G.getRes("land").amount<=60){
...
G.buyUnit=function(me,amount,any)
		...
		if (me.unit.wonder && amount>0)
		{
			...
			else if (me.mode==3 || me.mode==4)
			{
				...
				G.dialogue.popup(function(me,instance){return function(div){
					...
					if (instance.mode==3)
					{
						str+='<div class="fancyText par"><font color="'+(G.modsByName['Default dataset'] ? 'fuschia' : '#ccffcc')+'">This wonder only needs one more step to finalize.</font></div>';
	...
	G.setTab=function(tab)
	{
		if (tab.popup)
		{
			/*if(G.modsByName['Default dataset'] || G.modsByName['Elves'])
			if (G.checkPolicy('toggle SFX')=='on'){
						var audio = new Audio('https://orteil.dashnet.org/cookieclicker/snd/clickOff.mp3');
						audio.play(); 
						}*/
			if (G.getSetting('animations')) triggerAnim(tab.l,'plop');
			G.dialogue.popup(G.tabPopup[tab.id],'bigDialogue',tab.l);
